...
public static class CardPlayReplayPatch
{
    ...
    // SpireBot delta: private -> internal so the bot's stuck-state watchdog can reset the
    // combat gate (see StuckRecovery.cs / VENDORED-FROM.md). No behavior change.
    internal static void ClearEndTurnGate()
    {
        _awaitingEndTurnCompletion = false;
        _dispatching = false;
        _waitingForEffects = false;
