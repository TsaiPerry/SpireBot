...
public static class CardGridScreenCapture
{
    ...
    // FIX 1 (select.count): every NCardGridSelectionScreen subclass (NSimpleCardSelectScreen,
    // NDeckCardSelectScreen, NDeckUpgradeSelectScreen, NDeckTransformSelectScreen,
    // NDeckEnchantSelectScreen, NCombatPileCardSelectScreen) stores its own private
    // `CardSelectorPrefs _prefs` field (same name/type, declared per-subclass — there is no
    // shared base-class field) that CardSelectCmd populated from MinSelect/MaxSelect when the
    // screen was opened (see CardSelectCmd.cs's Prefs-taking Frome* methods). Reflect it off
    // the CONCRETE runtime type (cached per-Type) so RunObsWriter.WriteSelect can source
    // select.count the same way sts2-rl's run_env.py does (request.count_remaining, which
    // driver.py's `_card_selector` sets to `count - i` — at the FIRST ask (i=0, the only case
    // observed live) that's the full `count` == this screen's MaxSelect).
    private static readonly Dictionary<System.Type, FieldInfo?> PrefsFieldCache = new();

    internal static CardSelectorPrefs? GetPrefs(NCardGridSelectionScreen screen)
    {
        var t = screen.GetType();
        if (!PrefsFieldCache.TryGetValue(t, out var fi))
        {
            fi = t.GetField("_prefs", BindingFlags.NonPublic | BindingFlags.Instance);
            PrefsFieldCache[t] = fi;
        }
        return fi?.GetValue(screen) is CardSelectorPrefs prefs ? prefs : null;
    }
